...
void makeJets(std::string sample_name, Long64_t ievent, std::vector<Track> tracks, float R=0.8){
	...
	for (unsigned i = 0; i < jets.size(); i++) {
		...
		plotter.Plot1D(Form( "%s_jetsAK%i_jet_width", sample_name.c_str(),cone),";jet width.", jet_width, 100, 0, 2 );
	...
	plotter.Plot1D(Form( "%s_jetsAK%i_suep_jet_mass", sample_name.c_str(),cone),";mass.", SUEP_jet_mass, 100, 0, 10000 );
